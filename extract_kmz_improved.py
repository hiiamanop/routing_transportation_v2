#!/usr/bin/env python3
"""
Improved script to extract KMZ files including LineString routes
"""
...
def extract_kmz_enhanced(kmz_path):
    """Extract placemarks and linestrings from KMZ file"""
    stops = []
    routes = []
                    ...
                    if linestring is not None:
                        ...
                        for coord_str in coords_text.split():
                            ...
                            if len(parts) >= 2:
                                lon = float(parts[0])
                                lat = float(parts[1])
                                coord_list.append({'lon': lon, 'lat': lat})
...
def generate_stops_from_route(route_coords, csv_stops, koridor_name):
    """Generate stop names by matching CSV coordinates with route"""
    stops = []
    
    for i, csv_stop in enumerate(csv_stops):
        csv_lat = csv_stop['lat']
        csv_lon = csv_stop['lon']
        
        # Find nearest point on route
        min_distance = float('inf')
        nearest_route_idx = -1
        
        for j, route_point in enumerate(route_coords):
            distance = haversine_distance(csv_lat, csv_lon, route_point['lat'], route_point['lon'])
            if distance < min_distance:
                min_distance = distance
                nearest_route_idx = j
        
        # Create stop name based on position along route
        if min_distance < 200:  # Within 200m of route
            stop_name = f"{koridor_name} - Halte {i+1}"
        else:
            stop_name = f"{koridor_name} - Stop {i+1}"
...
def process_all_kmz_files():
    ...
    print("\n=== EXTRACTING ALL KMZ FILES (ENHANCED) ===\n")
    ...
    for transport_type, kmz_list in KMZ_FILES.items():
        ...
        for kmz_file in kmz_list:
            ...
            if kmz_path.exists():
                ...
                stops, routes = extract_kmz_enhanced(kmz_path)
                print(f"    Found {len(stops)} point stops and {len(routes)} route lines")
                all_kmz_data[transport_type][kmz_file] = {
                    'stops': stops,
                    'routes': routes
                }
            else:
...
def match_csv_with_kmz(csv_path, kmz_data, koridor_name):
    """Match CSV stops with KMZ data (stops or routes)"""
    ...
    try:
        ...
        # Get KMZ stops and routes
        kmz_stops = kmz_data.get('stops', [])
        kmz_routes = kmz_data.get('routes', [])
        ...
        for idx, row in df.iterrows():
            ...
            if pd.isna(existing_name) or existing_name == '' or existing_name.startswith('Stop '):
                # Try to find nearest from KMZ stops first
                if kmz_stops:
                    ...
                    if kmz_name:
                        ...
                        final_name = f"{koridor_name} - Halte {idx+1}"
                        print(f"    Created: '{final_name}' (no close match)")
                else:
                    # No point stops, use generic name
                    final_name = f"{koridor_name} - Halte {idx+1}"
                    if idx == 0:
                        print(f"    No point stops in KMZ, generating names from route...")
            else:
...
def main():
    print("=" * 80)
    print("PUBLIC TRANSPORT NETWORK BUILDER - ENHANCED")
    ...
    for i, csv_file in enumerate(CSV_FILES['feeder']):
        ...
        if kmz_file in all_kmz_data['feeder']:
            kmz_data = all_kmz_data['feeder'][kmz_file]
            matched = match_csv_with_kmz(csv_path, kmz_data, koridor_name)
            if matched:
    ...
    for i, csv_file in enumerate(CSV_FILES['teman_bus']):
        ...
        if kmz_file in all_kmz_data['teman_bus']:
            kmz_data = all_kmz_data['teman_bus'][kmz_file]
            matched = match_csv_with_kmz(csv_path, kmz_data, koridor_name)
            if matched:
                all_matched_stops.extend(matched)
    ...
    if kmz_file in all_kmz_data['lrt']:
        kmz_data = all_kmz_data['lrt'][kmz_file]
        matched = match_csv_with_kmz(csv_path, kmz_data, "LRT Sumsel")
        if matched:
            all_matched_stops.extend(matched)
    ...
    for route in network_data['routes']:
        route_stops = [n for n in network_data['nodes'] if n['route'] == route]
